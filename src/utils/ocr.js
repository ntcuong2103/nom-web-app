...
export async function recognizeCharacter(endpointUrl, imageFile, bbox, naturalWidth, naturalHeight) {
  if (!endpointUrl) throw new Error("OCR endpoint URL is required");
  if (!imageFile || !bbox) throw new Error("Image file and bounding box are required");
  ...
  return { text: result.text || "", ids: result.ids || "", confidence: result.confidence || 0 };
}

export async function recognizeCharactersBatch(endpointUrl, imageFile, bboxes, naturalWidth, naturalHeight) {
  if (!endpointUrl) throw new Error("OCR endpoint URL is required");
  if (!Array.isArray(bboxes) || bboxes.length === 0 || !imageFile) return [];
  const out = [];
  for (let i = 0; i < bboxes.length; i++) {
    try {
      const r = await recognizeCharacter(endpointUrl, imageFile, bboxes[i], naturalWidth, naturalHeight);
      out.push({ index: i, bbox: bboxes[i], ...r });
    } catch (e) {
      out.push({ index: i, bbox: bboxes[i], text: "", ids: "", confidence: 0, error: String(e) });
    }
  }
  return out;
}
